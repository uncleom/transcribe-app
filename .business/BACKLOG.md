...
- [x] **A2** Supabase Storage: приватный bucket — AUDIT.md §1.2 → фактически закрыто 2026-05-12 (bucket public→false, public read policy удалена).
...
- [x] **V1** Валидация `duration_hint`: проверка числа (>0, <14400, isFinite) в POST `/api/transcribe` → закрыто 2026-05-12.
- [ ] **M1** Реальный MercadoPago checkout для $5/$10/$20 пакетов — SPECIFICATION §5, MARKET_RESEARCH §5.

## P2 — следующий месяц

- [x] **A13** Polling max attempts: счётчик MAX=120 (~10 мин) в `/transcription/[id]` → закрыто 2026-05-12.
...
- [ ] **T1** Заменить Gladia на Groq Whisper Large v3 — после завершения тестирования. Groq дешевле и быстрее, diarization нужно проверить отдельно.
