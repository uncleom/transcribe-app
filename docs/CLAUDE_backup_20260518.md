...
**Обязательно** перед `git push origin main`:
```bash
ssh root@162.55.215.194 "docker builder prune -af && docker image prune -af"
```
Причина: Hetzner 38GB, Docker build cache ~2GB за деплой — диск заполняется быстро.
Docker GC (daemon.json) не справляется при интенсивной разработке.

## /plan
Когда пользователь пишет **/plan**:
1. Прочитай `ARCHITECTURE.md` и `CONTEXT_MAPS.md` (если существует)
2. Задай один вопрос: «Какую фичу хочешь реализовать?»
3. Дождись ответа, затем создай пошаговый план реализации:
   - Какие файлы затрагиваются и почему
   - Порядок шагов с зависимостями между ними
   - Потенциальные риски и нетривиальные решения
4. **Не пиши код.** Только план — файлы, шаги, риски.
...
Groq API (резюме + перевод — модель llama-3.3-70b-versatile)
next-pwa 5.6.0 (сконфигурирован, SW генерируется через `next build --webpack`)
Telegram Bot API (webhook, inline keyboards) — @TranscriboAppBot
...
telegram_accounts:     telegram_id (PK), supabase_user_id, telegram_username, created_at
telegram_link_tokens:  token (PK), supabase_user_id, expires_at, used, created_at
```
...
- ✅ Telegram bot (@TranscriboAppBot): транскрипция, summary, перевод, /history, линковка аккаунта
- ✅ `/connect-telegram` страница + TelegramLinkButton (генерация one-time deep link)
- ✅ Карточка Telegram на главной + блок в /billing
- ✅ Единый пайплайн транскрипции в `src/lib/transcription.ts` (сайт + бот)
- ✅ GitHub репозиторий публичный: github.com/uncleom/transcribe-app
- ✅ README обновлён под портфолио с Telegram-фичей
...
GROQ_API_KEY
NEXT_PUBLIC_APP_URL
TELEGRAM_BOT_TOKEN       # @TranscriboAppBot
TELEGRAM_SECRET_TOKEN    # webhook validation secret
COOLIFY_API_KEY          # в .env.local (не в репо)
```
